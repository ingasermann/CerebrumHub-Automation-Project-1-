...
Assignment 2:
 ...
 1. Update the name of test suite by adding you name: “This is first test suite, Inga Sermann”
...
describe('This is first test suite, Inga Sermann', () => {
    // Test created on: July 20, 2024
    it('User can submit data only when valid mandatory values are added', () => {
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('input[name="password"]').type('WordPass765')
        cy.get('[name="confirm"]').type('WordPass765')
    ...
    */
    ...
    it('User cannot submit data when phone number is absent', () => {
        ...
        // Assert that phone number error message is visible
        cy.get('#input_error_message').should('be.visible').should('contain', 'Mandatory input field is not valid or empty!')
    })
    ...
    it('User cannot submit data when password and/or confirmation password is absent', () => {
        ...
        // Assert that password error message is visible
        cy.get('#password_error_message').should('be.visible').should('contain', 'Mandatory input field is not valid or empty!')
    })
    ...
    it('User cannot add letters to phone number', () => {
        ...
        cy.get('#username').type('johnDoe')
        cy.get('[data-testid="phoneNumberTestId"]').should('have.attr', 'type', 'number').type('abcde')
        cy.get('input[name="password"]').type('Password123')
        cy.get('[name="confirm"]').type('Password123')
